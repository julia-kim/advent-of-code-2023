...
export function part1(input?: string[]) {
  ...
  lines.forEach((line, index) => {
    if (!line || line.length === 1) { next = true; return }
    if (index == 0) { seeds = line.replace("seeds: ", "").trim().split(" ").map(s => { return +s }); return }
    if (next == true) {
    ...
    if (next == false) {
      const [dest, source, length] = line.split(" ").map(value => { return +value })
